...
func TestSearchAddressDetail(t *testing.T) {
	var (
		req = "placeId"
		res = &dto.SearchAddressDetailResponse{}
	)
	tests := []struct {
		name       string
		req        string
		...
		{
			name: "should return list of address detail with code 200 when search address detail success",
			req:  req,
			want: response.Response{
				Code:    code.OK,
				Message: "success",
			...
			code: http.StatusOK,
			beforeTest: func(addressService *mocks.AddressService) {
				addressService.On("GetSearchAddressDetail", req).Return(res, nil)
			},
		},
		{
			name: "should return error with code 500 when search address detail failed",
			req:  req,
			want: response.Response{
				Code:    code.INTERNAL_SERVER_ERROR,
				Message: errs.ErrInternalServerError.Error(),
			...
			code: http.StatusInternalServerError,
			beforeTest: func(addressService *mocks.AddressService) {
				addressService.On("GetSearchAddressDetail", req).Return(nil, errs.ErrInternalServerError)
			},
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			req, _ := http.NewRequest(http.MethodGet, fmt.Sprintf("/v1/locations/addresses/%s", test.req), nil)
			_, rec := testutil.ServeReq(cfg, req)
